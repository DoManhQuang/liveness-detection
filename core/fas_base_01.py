...
def block_conv(x, filter_block, activation='relu', padding='same', name="block_conv"):
    ...
    conv_a = Conv2D(filter_block, kernel_size=7, strides=1, activation=activation, padding=padding)(conv_a)
    ...
    conv_b = Conv2D(filter_block, kernel_size=5, strides=2, activation=activation, padding=padding)(conv_b)
    ...
    conv_c = Conv2D(filter_block, kernel_size=3, strides=1, activation=activation, padding=padding)(conv_c)
    conv_c = BatchNormalization()(conv_c)

    conv_d = Conv2D(filter_block, kernel_size=1, strides=2, activation=activation, padding=padding)(x)
...
def created_model_fas_01(input_shape, number_class, activation):
    ...
    x = block_identity_group(x=x, filter_blocks=[16, 32, 64], activation='relu', padding='same', name="block_identity_group_a")
